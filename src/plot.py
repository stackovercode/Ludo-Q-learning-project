...
for i, LRval in enumerate(learning_rate):
    ax = fig.add_subplot(len(learning_rate), 1, i+1, projection='3d')

    # Create a 2D array for this learning rate
    avg_win_rate_2D = avg_win_rate[:, :, i]

    print(avg_win_rate_2D)
    

    print(avg_win_rate_2D.shape)

    BT, DF = np.meshgrid(boltzmann_temperature, discount_factor)

    ax.plot_surface(BT, DF, avg_win_rate_2D, cmap='viridis')
    ...
    ax.set_title(f'Learning Rate = {LRval}')
