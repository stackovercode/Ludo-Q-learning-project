...
class QLearning:
    ...
    def learn(self, game, dice, player_pieces, enemy_pieces):
        self.update_q_table(player_pieces, enemy_pieces, dice, game, game.there_is_a_winner())

        current_states = self.determined_state(player_pieces, enemy_pieces, game)
        possible_actions = self.determined_possible_actions(player_pieces, enemy_pieces, dice)

        reward = self.get_reward(player_pieces, current_states, game.there_is_a_winner())

        if self.last_action is not None:
            max_q_next_state = np.max(self.Q_table[current_states])

            self.Q_table[self.last_state, self.last_action] += self.learning_rate * (
                reward + self.discount_factor * max_q_next_state - self.Q_table[self.last_state, self.last_action]
            )

        if game.there_is_a_winner():
            if game.get_winner() == self.player_index:
                self.number_of_wins += 1
            self.reset_game()
        else:
            self.current_state = current_states
            self.last_player_pieces = player_pieces.copy()
            self.last_state = current_states[self.pick_action(current_states, possible_actions)]
            self.last_action = self.pick_action(current_states, possible_actions)
